...
class Spider(Spider):  # 元类 默认的元类 type
	...
	def fhdContent(self,flag,id,vipFlags):
		...
		if 'location' in noRsp.headers and len(realUrl) == 0 :
			realUrl = noRsp.headers['location']
		newHeader = {
			"user-agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/99.0.4606.54 Safari/537.36",
	...
	def originContent(self,flag,id,vipFlags):
		...
		newHeader = {
		        "user-agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.49.36",
		        "referer":"https://www.aliyundrive.com/",
		}
	...
	def detailContent(self,array):
		...
		infoRsp = requests.post(infoUrl,json = infoForm,headers=customHeader)
		...
		vodList = {
			'vod_id':tid,
			'vod_name':infoJo['share_name'].replace('#',''),
	...
	authorization = ""
	#authorization = requests.get("http://192.168.123.150:81/www/tvbox/alick.txt").text.replace('\n','')
	#authorization = requests.get("https://cjk.lm317379829.repl.co/CJK/token.txt").text.replace('\n','')
	timeoutTick = 0
	localTime = 0
	expiresIn = 0
	shareTokenMap = {}
	...
	def login(self):
		...
		alitoken = requests.get("http://192.168.123.150:81/www/tvbox/alick.txt").text.replace('\n','')
                
		#alitoken = requests.get("https://cjk.lm317379829.repl.co/CJK/token.txt").text.replace('\n','')
