...
async function detail(id) {
    ...
    if (isJustLive) {
        const headers = {
            'Content-Type': 'application/x-www-form-urlencoded', 'User-Agent': MOBILE_UA,
        };
...
async function search(wd, quick) {
    ...
    for (const vod of data.response['3'].docs) {
        videos.push({
            vod_id: vod.room_id, vod_name: vod.game_roomName, vod_pic: vod.game_screenshot, vod_remarks: vod.game_nick,
        });
    }
    return JSON.stringify({
        list: videos,
