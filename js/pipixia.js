...
class PiPiXiaSpider extends Spider {
    ...
    async parseVodShortListFromDoc($) {
        ...
        for (const vodElement of vodElements) {
            ...
            vodShort.vod_pic = this.baseProxy + Utils.base64Decode(this.siteUrl + $(vodElement).find("[class=\"lazy gen-movie-img mask-1\"]")[0].attribs["data-original"])
    ...
    async parseVodShortListFromDocBySearch($) {
        ...
        for (const vodElement of vodElements) {
            let vodShort = new VodShort();
            vodShort.vod_pic = this.baseProxy + Utils.base64Decode(this.siteUrl + Utils.getStrByRegex(/url\((.*?)\);/, $(vodElement).find("[class=\"cover\"]")[0].attribs.style))
    ...
    async parseVodShortListFromJson(obj) {
        ...
        for (const vod_json of obj["list"]) {
            ...
            vodShort.vod_pic = this.baseProxy + Utils.base64Decode(this.siteUrl + vod_json["vod_pic"])
