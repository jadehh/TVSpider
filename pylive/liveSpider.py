...
class LiveSpider(object):
    ...
    def downloadTsUrl(self,segements):
        ## 计算下载速度,和 ts文件的分辨率
        startTime = time.time()
        totalSize = 0
        index = 1
    ...
    def downloadM3U8Url(self, m3u8Url):
        ...
        if res:
            if b"EXTM3U" in res:
                ...
                if len(m3u8Object.segments) > 0 and len(m3u8Object.segments) < 10:  ## 防止有不是直播文件
    ...
    def selectBestUrl(self, name, m3u8List):
        ...
        for (thread,m3u8Url) in threadList:
            ...
            if speed > 0:
                JadeLog.INFO("播放连接为:{},速度为:{}MB/s,分辨率为:{},测试完成".format(m3u8Url, ("%.2f" % speed), aspect))
            else:
