...
class User {
    ...
    async save() {
        if (server !== null) {
            await server.db.push(server.prefix + '/device' + "ali_aliyundrive_user", this.toDict());
        } else {
...
class OAuth {
    ...
    async save() {
        if (server !== null) {
            await server.db.push(server.prefix + '/device' + "ali_aliyundrive_oauth", this.toDict());
        } else {
...
async function getUserCache(serverConfig) {
    server = serverConfig
    if (server !== null) {
        return JSON.stringify(await server.db.getObjectDefault(server.prefix + '/device' + "ali_aliyundrive_user", {}))
    } else {
...
async function getOAuthCache() {
    if (server !== null) {
        return JSON.stringify(await server.db.getObjectDefault(server.prefix + '/device' + "ali_aliyundrive_oauth", {}))
    } else {
