...
class DanmuSpider {
    ...
    async parseVodShortListFromJson(obj, vodDetail) {
        for (const componentObj of obj["pageComponentList"]) {
            if (componentObj["commonData"] !== undefined) {
                ...
                if (searchVodDetail.type_name.indexOf(vodDetail.vod_year) > -1 && searchVodDetail.type_name.indexOf(vodDetail.type_name) > -1 || searchVodDetail.vod_director === vodDetail.vod_director) {
                    await this.jadeLog.debug("匹配视频网站成功", true)
    ...
    async downloadDanmu(url) {
        ...
        let xml = String.raw`<?xml version="1.0" encoding="UTF-8"?><i><chatserver>chat.bilibili.com</chatserver><chatid>52175602</chatid><mission>0</mission><maxlimit>1000</maxlimit><state>0</state><real_name>0</real_name><source>k-v</source>`
            + list.join('') + "</i>";
        await this.jadeLog.debug(`弹幕结果为:${xml}`)
        let savecontent = await req(`http://127.0.0.1:9978/cache?do=set&key=danmu&value=${xml}`)
        await this.jadeLog.debug(`使用TV存储,返回结果为:${JSON.stringify(savecontent)}`)
        let content= await req("http://127.0.0.1:9978/cache?do=get&key=danmu")
        await this.jadeLog.debug(`TV缓存,弹幕结果为:${JSON.stringify(content)}`)
