...
async function alist(url_param, params) {
    let url = "https://api-cf.nn.ci/alist/ali_open/" + url_param, response = await postJson(url, params, getHeader()),
...
async function getVod(video_item_list, sub_item_list) {
    ...
    for (const video_item of video_item_list) episode_list.push(video_item.getDisplayName() + "$" + video_item.getFileId() + "+" + video_item.shareId + "+" + video_item.shareToken + findSubs(video_item.getName(), sub_item_list));
        ...
        if (r_item.getType() === "folder") {
            ...
            if ((r_item.getCategory() === "video" || r_item.getCategory() === "audio")) {
                ...
                if (!is_video_file_exists) {
                    if (r_item.getCategory() === "video" && r_item.size / 1000000 > 1) {
...
async function copy(file_id, shareId, shareToken) {
    ...
    if (typeof (copy_file_id) == "string") {
        ...
        if (response.code === 500 || response.code === 501 || response.code === 502 || response.code === 403) {
