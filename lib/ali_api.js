...
async function getVod(video_item_list, sub_item_list) {
    ...
    for (const video_item of video_item_list)
        episode_list.push(video_item.getDisplayName() + "$" + video_item.getFileId()   + "+" + video_item.shareId + "+" + video_item.shareToken + findSubs(video_item.getName(), sub_item_list)) ;
...
async function listFilesMarker(item, video_item_list, sub_item_list, netxt_markers,share_token) {
    ...
    for (const r_item of items.getItems()) {
        if (r_item.getType() === "folder") {
            ...
            if ((r_item.getCategory() === "video" || r_item.getCategory() === "audio") && (r_item.name !== "0-4k玩偶哥哥.mp4") && (r_item.name !== "00-4K.mp4")) {
                //判断数组中是否有file_id
                //
