...
let quality = {},
    ...
    tmpFolderName = "TV",
    curTmpFolderFileId = "",
    JadeLog = new JadeLogging("阿里云盘", "DEBUG");
...
async function clearCacheFile() {
    local.set("file", "file_id", JSON.stringify({}))
}
...
async function alist(url_param, params) {
    let url = "https://api-cf.nn.ci/alist/ali_open/" + url_param,
        response = await postJson(url, params, getHeader()),
        response_content = response.content;
    if (await isManyRequest(response_content) || _.isEmpty(response_content)) {
        if (_.isEmpty(response_content)) {
            await JadeLog.error(`Alist授权Token失败,失败原因为:太多请求`)
        } else {
...
// 阿里云盘用户Api
async function auth(url, params, retry) {
    url = url.startsWith("https") ? url : "https://api.aliyundrive.com/" + url;
    let response = await postJson(url, params, getHeaderAuth());
    await JadeLog.debug(`正在请求需要阿里登录的url:${url},参数为:${JSON.stringify(params)}`)
    if (retry && (response.code === 400 || response.code === 401 || response.code === 429)) {
        if (response.content.indexOf("AccessTokenInvalid") > -1) {
            await JadeLog.error("登录阿里云盘失败,失败原因为:登录Token无效,准备重新授权,失败详情:" + response.content)
        } else {
            await JadeLog.error("登录阿里云盘失败,失败原因为:未知,准备重新授权 Token,失败详情:" + response.content)
        }
        await refreshAccessToken("")
        return await auth(url, params, false);
    }
...
async function oauthFunc(url, params, retry) {
    ...
    let response = await postJson(url, params, getHeaderOpen());
    if (retry && (response.code === 400 || response.code === 401)) {
        if (response.content.indexOf("AccessTokenInvalid") > -1) {
            await JadeLog.error("阿里授权失败,失败原因为:授权Token无效,准备重新授权,失败详情:" + response.content)
        } else if (response.content.indexOf("AccessTokenExpired") > -1) {
            await JadeLog.error("阿里授权失败,失败原因为:授权Token失效,准备重新授权,失败详情:" + response.content)
        } else {
            await JadeLog.error("阿里授权失败,失败原因为:未知,准备重新授权,失败原因为:" + response.content)
        }
        await activateRefreshOpenToken()
        return await oauthFunc(url, params, false)
    }
    if (retry && response.code === 429) {
        sleep(1000)
        await JadeLog.error(`正在请求需要阿里授权的url:${url},请求过于频繁,稍后重试}`)
        return await oauthFunc(url, params, true)
    }
    await JadeLog.debug(`完成请求需要阿里授权的url:${url},参数为:${JSON.stringify(params)},请求码为:${response.code.toString()},请求结果为:${response.content}`)
    return response.content;
}
...
async function isManyRequest(data_str) {
...
async function refreshAccessToken(token) {
    try {
        if (_.isEmpty(token) || _.isEmpty(user.getAccessToken()) || user.getRefreshToken() !== token) {
            ...
            await JadeLog.info("阿里云盘已登录,无需重复登录")
        }
        return true;
    } catch (e) {
...
async function oauthRequest() {
    try {
        ...
        let response_str = await auth(url, params, true);
...
async function getVod(share_url, file_id) {
    await JadeLog.info(`正在获取播放链接,分享链接为:${share_url},文件id为:${file_id}`)
    let params = {};
    params.share_id = shareId;
    ...
    await listFiles(item, video_item_list, sub_item_list);
    let play_foramt_list = ["原画", "超清", "高清", "标清"],
        episode_list = [],
        episode_str_list = [];
    for (const video_item of video_item_list)
        episode_list.push(video_item.getDisplayName() + "$" + video_item.getFileId() + findSubs(video_item.getName(), sub_item_list));
    for (let index = 0; index < play_foramt_list.length; index++) {
        episode_str_list.push(episode_list.join("#"));
    }
    if (episode_str_list.length > 0) {
        await JadeLog.info(`获取播放链接成功,分享链接为:${share_url},文件id为:${file_id}`)
    } else {
        await JadeLog.error(`获取播放链接失败,检查分享链接为:${share_url},文件id为:${file_id}`)
    }
    return {
...
async function listFiles(item, video_item_list, sub_item_list) {
    return await listFilesMarker(item, video_item_list, sub_item_list, "");
}

async function listFilesMarker(item, video_item_list, sub_item_list, netxt_markers) {
    ...
    let items = Item.objectFrom(await shareFunc("adrive/v2/file/list_by_share", new_item));
    for (const r_item of items.getItems()) {
        if (r_item.getType() === "folder") {
            ...
            if (r_item.getCategory() === "video" || r_item.getCategory() === "audio") {
                ...
                Utils.isSub(r_item.getExt()) && sub_item_list.push(r_item);
            }
        }
    }
...
async function getSubs(sub_list) {
    let sub_url_list = [];
    for (const sub_str of sub_list) {
        ...
        let sub_split_list = sub_str.split("@@@"),
            ...
            sub_url = await getDownloadUrl(sub_file_id);
...
async function playerContent(file_id_list) {
    try {
        await JadeLog.info("正在获取原画的播放地址和字幕下载链接", true)
        let download_url = await getDownloadUrl(file_id_list[0]),
            sub_list = await getSubs(file_id_list),
            sub_url_list = [];
        for (const sub of sub_list) {
            sub_url_list.push(sub.url);
        }
...
// 转码头的Url和字幕
async function playerContentByFlag(file_id_list, flag) {
    try {
        await JadeLog.info("正在获取转码后的播放地址和字幕下载链接", true)
        let video_preview_play_info = await getVideoPreviewPlayInfo(file_id_list[0]),
...
async function copy(file_id) {
    let copy_file_id
    let cache_dic = JSON.parse(await local.get("file", "file_id"))
    ...
    if (typeof (copy_file_id) == "string") {
        ...
        let response_str = await auth("adrive/v2/batch", JSON.parse(params_str), true);
        await JadeLog.debug(`转存文件成功,文件id为:${file_id},请求结果为:${response_str}`)
        if (response_str.indexOf("ForbiddenNoPermission.File") > -1) {
            return copy(file_id);
        }
        copy_file_id = JSON.parse(response_str).responses[0].body.file_id;
        let file_dic = JSON.parse(await local.get("file", "file_id"))
        file_dic[file_id] = copy_file_id
        local.set("file", "file_id", JSON.stringify(file_dic))
    }
    return copy_file_id;
}
...
async function trashFile(file_id) {
    ...
    let response = await auth("v2/batch", JSON.parse(params_str), true);
...
async function recyclebinClear() {
    ...
    await auth("v2/recyclebin/clear", params, true);
