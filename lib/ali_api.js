...
function aliExpection(data_str) {
    ...
    }else if (data_str.indexOf("ForbiddenNoPermission.File")){
         return {code: 500, content: data_str}
    }else if (data_str.indexOf("InvalidParameter.ToParentFileId")){
...
async function oauthFunc(url, params, retry) {
    ...
    response = aliExpection(response.content)
    if (retry && (response.code === 400 || response.code === 401 || response.code === 429 || response.code === 402)) {
...
async function getFileByShare(share_url, video_item_list, sub_item_list) {
    ...
    let response_str = await post("adrive/v3/share_link/get_share_by_anonymous", params),
        ...
        item_file_id = getParentFileId("", response_json),
        item = new Item(item_file_id);
    await listFiles(item, video_item_list, sub_item_list);
}

async function getVod(video_item_list, sub_item_list) {
...
async function copy(file_id) {
    ...
    if (typeof (copy_file_id) == "string") {
        ...
        if (response.code === 500 || response.code === 501) {
...
export {initSome, setToken, clearFile, setShareId, getFileByShare, getVod, playerContent, playerContentByFlag};