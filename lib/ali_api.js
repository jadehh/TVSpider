...
async function initSome(serverConfig) {
    server = serverConfig
    let user_cache_str = await getUserCache(serverConfig);
...
async function cleanRecord() {
    if (server !== null) {
        await server.db.push(server.prefix + '/device' + "ali_file_id", {})
    } else {
...
async function copy(file_id, shareId, shareToken) {
    ...
    try {
        if (server !== null) {
            cache_dic = await server.db.getObjectDefault(server.prefix + '/device' + "ali_file_id", {});
        } else {
    ...
    if (typeof (copy_file_id) == "string") {
        ...
        try {
            if (server !== null) {
                await server.db.getObjectDefault(server.prefix + '/device' + "ali_file_id", {});
            } else {
        ...
        if (server !== null) {
            await server.db.push(server.prefix + '/device' + "ali_file_id", file_dic)
        } else {
