...
let db = null
async function initSome(database) {
    db = database
    let user_cache_str = await getUserCache(database);
...
async function cleanRecord() {
    if (db !== null){
        await this.db.push("ali_file_id", {})
    }else{
...
async function copy(file_id, shareId, shareToken) {
    ...
    try {
        if (db !== null){
            cache_dic =  await db.getObjectDefault("ali_file_id", {});
        }else{
    ...
    if (typeof (copy_file_id) == "string") {
        ...
        try {
            if (db !== null){
                await db.getObjectDefault("ali_file_id", {});
            }else{
        ...
        if (db !== null){
            await db.push("ali_file_id",file_dic)
        }else{
